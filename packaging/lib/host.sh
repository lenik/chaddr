...
#
# Environment:
#   CHADDR_SRCDIR       project root (default: packaging/../..)
#   CHADDR_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
CHADDR_SRCDIR=${CHADDR_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$CHADDR_SRCDIR/.config/chaddr" "$HOME/.config/chaddr"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${CHADDR_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${CHADDR_PACKAGING:-}")"
...
cmd_run() {
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $CHADDR_SRCDIR/.config/chaddr/$k.build-host" >&2
        echo "    $HOME/.config/chaddr/$k.build-host" >&2
    done
    return 2
}
