# chaddr
`chaddr` 是一个 Python + Meson 命令行应用项目模板。  
`chaddr` 是此模板中的一个**示例应用**；同一仓库中可以继续添加更多应用。

## 仓库结构

- `src/` - Python 源码（示例 `chaddr.py` 与共享辅助模块 `commons.py`）
...
`chaddr` 使用 `po/` 下的 gettext 翻译文件（`*.po` 与生成的 `.mo` 文件）。
...
- 开发态运行（`/build/chaddr`）若存在 `/build/po`，会优先使用项目内翻译资源。
...
```bash
LANGUAGE=ja /build/chaddr -h
LANGUAGE=zh_CN /build/chaddr -h
```
